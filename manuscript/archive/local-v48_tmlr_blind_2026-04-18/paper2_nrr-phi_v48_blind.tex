...
% TITLE AND AUTHOR
%==============================================================================
\title{\textbf{Text-to-State Mapping for Ambiguity Preservation in LLM Inference}}
...
%==============================================================================
\begin{abstract}
Ambiguity-bearing inputs often reach downstream systems through interfaces that favor a
single resolved response before later context arrives. This creates a representational
gap: multiple plausible interpretations may be expressible, but they are not preserved
as one manipulable state.
...
and state construction. We instantiate $\phi$ with a hybrid extraction design that combines
rule-based segmentation for explicit conflict markers (adversative conjunctions, hedging expressions)
with LLM-assisted enumeration of implicit ambiguity (epistemic, lexical, structural).
...
compared to $H = 0$ for single-interpretation collapse baselines used as reference points.
...
conflict detection stage. This framework extends a prior retained-state formulation by providing
the missing algorithmic bridge between text and the retained state space, enabling architectural
...
\textbf{Package surface}: The current repository bundles deterministic reruns for the
rule-based mapping and operator-validation appendix, together with archived prompt/output
artifacts and a transcript-audit path for the LLM-handled ambiguity cases reported here.
\end{abstract}

\noindent\textbf{Keywords:} text-to-state mapping, retained-state inference, collapse deferment,
...
This paper develops a text-to-state mapping layer for ambiguity-preserving retained-state inference. It focuses on how competing interpretations are mapped into a retained state and how that retained state can later support non-collapsing updates.
...
understanding and generation. However, deployed inference interfaces typically return
one resolved response, which creates pressure toward early commitment when context is
still incomplete.

Consider the input: ``I want to quit my job, but I also don't want to quit.'' 
A standard one-response interface must produce an output, which pushes the system toward
one interpretation of this ambivalent statement. Typical responses either:
\begin{itemize}
...
Each of these responses treats the ambivalence as something to resolve. They may
enumerate alternatives in text, but they do not by themselves maintain both
interpretations as one persistent state for later operators.

This \textit{early semantic commit} pressure is encouraged by common architectural
choices in standard LLM design:
...
\subsection{Retained-State Reasoning}

Prior retained-state work \cite{anon2025framework} offers an alternative paradigm that
treats ambiguity preservation as a valid computational mode. That framework introduced the state
space $\mathcal{S} = \{(v_i, c_i, w_i, m_i)\}$ where multiple interpretations coexist with
independent activation weights, and demonstrated that a two-path retained-state prototype
...
into the retained state space was described conceptually but not algorithmically specified.
...
\begin{enumerate}
    ...
    \item \textbf{Hybrid Extraction Design}: We combine rule-based segmentation
          (for explicit conflict markers) with LLM-assisted enumeration (for implicit
          ambiguity), pairing deterministic executable coverage for explicit-marker
          cases with archived transcript-audit support for the implicit-ambiguity
          cases reported here.
    
    \item \textbf{Experimental Validation}: We evaluate a hybrid 68-sentence test set,
          combining three-LLM extraction on 28 English sentences (ChatGPT, Gemini,
          Claude) with rule-based extraction on the remaining explicit-marker cases,
          and obtain overall entropy preservation ($H = 1.087$) across ambiguity
          categories.
...
Contribution type and claim boundary are explicit here as well. The paper does not prove that retained multiplicity is always cheaper, nor that all future retained-state systems have one fixed runtime class. Its narrower algorithmic contribution is to show how text can be mapped into one manipulable retained state so updates need not default to explicit branch re-enumeration and full comparative checking of every live alternative at each step.

\paragraph{Support structure.}
The evidence surface is intentionally layered. Layer 1 is the formal construction
showing how text maps into retained state representations. Layer 2 is the current
empirical surface: deterministic rule-based reruns, deterministic operator-validation
reruns, and transcript-audited summaries for the LLM-handled ambiguity cases used in
the combined table. A direct semantic-adequacy layer for the LLM-handled cases is not
part of the present package and is therefore not claimed as part of the core evidence.
...
the retained-state transition operators and metadata functions that transform states while preserving non-collapse.

\subsection{Notation and Metric Contracts}

\textbf{Notation contract.} In this paper, $\sigma_{\mathrm{state}}$ denotes a
state-level calibration operator in Appendix~\ref{app:operators}. This is
distinct from item-level strengthen/dampen operators used in later implementation
papers. The symbol family is consistent; the operation level differs.

\textbf{Metric contract.} Phi primarily evaluates text-to-state validity and
non-collapse properties (state entropy and violation rates). Later
implementation papers add interface/operational metrics such as extraction
reliability and token efficiency. Cross-paper interpretation should treat these
as layered metrics across stages, not as competing definitions of success.

\textbf{Boundary statement.} Claims are limited to tested datasets and operator
validation settings in this paper.
In this paper, validity is condition-indexed rather than unconditional.
A mapped interpretation is valid under its associated context tag, not universally valid across contexts.
...
representational end state. The retained-state approach inverts this relationship: non-collapse is the goal,
...
\subsection{Relation to the Retained-State Framework}

This paper builds directly on prior retained-state work \cite{anon2025framework}. That work established
the state space formalism and demonstrated entropy preservation through functional verification (a two-path retained-state prototype achieving $H = 0.91$ bits vs.\ baseline $H = 0.15$ bits).
The framework defines a transition-oriented core operator set ($\delta, \tau, \kappa, \pi$), with
item-level strengthening used in later implementations, and additional
metadata functions ($\alpha, \rho, \iota$) for representation management
(see Appendix~\ref{app:operators} for summary).

This paper fills the gap between text and state space: without a defined $\phi$,
the retained-state operators have no input to operate on. We provide that input transformation.
...
Following \cite{anon2025framework}, a \textit{semantic state space} is a structure
...
and $\tau \in (0,1)$ is a conceptual deduplication threshold describing the
merge contract. The current paper does not treat one fixed executed $\tau$
value as a separately validated package claim.
...
The factor $\beta > 0$ schematically increases activation for
interpretations from conflicting contexts, reflecting the retained-state principle
that conflict indicates information density. In the present paper, $\beta$
specifies the construction form rather than a separately audited executable
package constant.
...
\begin{proposition}[Non-Collapse Construction Property of $\phi$]
...
\begin{remark}
The proposition records a \textit{relative} construction property: when
multiple interpretations are extracted, $\phi$ preserves more information
than collapse-based alternatives. The absolute entropy depends on extraction
quality and weight calibration, addressed empirically in
...
% SECTION 4: ALGORITHM
%==============================================================================
\section{Algorithmic Form and Package Surface}
\label{sec:algorithm}

\subsection{Text-to-State Mapping Algorithm}

\begin{algorithm}[h]
\caption{Conceptual Text-to-State Mapping ($\phi$)}
\label{alg:phi}
\begin{algorithmic}[1]
\REQUIRE Text $T$, embedding function Embed, optional auxiliary interpretation source
...
\ENDIF
\STATE $I_{\text{aux}} \gets \text{EnumerateImplicitAmbiguity}(T, \mathbf{f})$
\STATE $I \gets \text{MergeContract}(I_{\text{rule}}, I_{\text{aux}})$
...
\FOR{$(\text{interp}, \text{ctx}, \text{conf}) \in I$}
    ...
    \STATE $w \gets \text{AssignWeight}(\text{conf}, \text{has\_conflict})$
...
Algorithm~\ref{alg:phi} describes the hybrid mapping contract at the design level.
The current package exposes deterministic reruns for the explicit-marker rule-based
mapping and for Appendix~\ref{app:operators} operator validation. For the implicit
ambiguity categories in Section~\ref{sec:experiments}, the package exposes archived
prompt/output transcripts, a sentence-level manifest, and a local audit script that
reconstruct the reported summaries rather than a fully rerunnable live LLM harness.

\subsection{Retained-State Processing Pipeline}

The mapping $\phi$ feeds into the retained-state operator pipeline (Appendix~\ref{app:operators}):

\begin{algorithm}[h]
\caption{Retained-State Processing Pipeline}
\label{alg:pipeline}
\begin{algorithmic}[1]
...
\STATE $S_0 \gets \rho(S_{\text{new}})$ \COMMENT{Optional metadata tagging (weights unchanged)}
\STATE $S_1 \gets \tau(S_0)$ \COMMENT{Hold/defer: keep ambiguity as-is for this turn}
\STATE $S_2 \gets \delta(S_1, \lambda)$ \COMMENT{Dampen convergence}
\STATE $S_3 \gets \kappa(S_2, S_{\text{prev}})$ \COMMENT{Integrate with history (CPP)}
\STATE $S' \gets \pi(S_3, S_{\text{prev}})$ \COMMENT{Apply temporal persistence/decay}
...
In this pipeline, $\tau$ is intentionally a no-op on weights: it marks
an explicit defer/hold decision instead of forcing an update.
...
\noindent Note that Proposition~\ref{thm:noncollapse} requires $|\mathcal{I}(T)| > 1$
...
($|\mathcal{I}| = 1$), so the proposition's precondition is not met. The $H = 0$
...
To address the limitations of rule-based extraction, we audited archived
LLM-generated interpretation sets from three models: ChatGPT, Gemini, and Claude.\footnote{Experiments used free-tier web interfaces in January 2026; exact model build IDs were not exposed.}
Each archived run enumerated possible interpretations for 28 English
sentences (8 epistemic + 10 lexical + 10 structural). The repository bundles
the prompts, saved outputs, and audit manifest used to reconstruct these
summaries from \texttt{prompts/}.
...
\bottomrule
\end{tabular}
\caption{Archived LLM extraction summaries (English only) across three models. All
categories yield $H > 0$ in the audited outputs, with epistemic showing the
highest entropy due to multiple interpretive layers (assertion vs. hedge vs.
self-justification).}
...
\caption{Archived LLM extraction summaries across three models (ChatGPT, Gemini,
Claude) for English sentences. The dashed line indicates $H_{\max} = 1.0$ for
binary interpretations. All audited model outputs yield $H > 0$ across all
categories, with epistemic showing the highest entropy due to multiple
interpretive layers.}
...
Within the archived outputs, LLM-assisted extraction recovers interpretive
multiplicity for all categories that the current rule-based methods do not
cover:
\begin{itemize}
...
Within this archived set, Claude extracted the most interpretations on average
($|S| = 3.75$ for epistemic), while all three models yielded $H > 0$ for
100\% of test sentences.

\subsection{Combined Results}

Table~\ref{tab:results-combined} presents the combined results surface, combining rule-based
extraction (for adversative and hedging, including Japanese sentences) with
audited archived LLM extraction summaries (for epistemic, lexical, and structural,
English only).
...
\bottomrule
\end{tabular}
\caption{Combined results using the reported hybrid evidence surface. All categories
achieve $H > 0$, with overall mean entropy $H = 1.087$ bits. The overall row is
computed as the weighted mean across all 68 sentences, where LLM-based $H$
values represent the three-model average for each sentence in the archived set.}
...
with $H \approx 1.0$. For the implicit-ambiguity categories in this paper's
archived LLM set, the audited outputs yield $H = 0.97$--$1.67$.
...
\textbf{(RQ4)} In the archived three-model set, all three LLMs (ChatGPT, Gemini,
Claude) yield $H > 0$ for 100\% of English test sentences. Claude extracts the
most interpretations (mean $|S| = 3.75$ for epistemic), while Gemini is more
conservative ($|S| = 3.00$). Despite variation in $|S|$, all audited outputs
preserve interpretive multiplicity on this dataset.

\subsection{Limitations}

While the experimental results support the framework, several limitations remain:
...
\noindent\textbf{Note on LLM role:} LLMs serve here as archived sources of interpretation enumeration; the theoretical claims of this paper concern the structure of the resulting states, not the capabilities of the underlying models. The $\phi$ mapping framework is model-agnostic and could be instantiated with any interpretation source.
This is a derivation-and-evaluation framework, not a closed recipe; the same derivation lens supports alternative mapping/operator instantiations under fixed protocols with condition-bounded reporting.
...
Off-the-shelf assistant responses often tend to:
\begin{itemize}
...
This application is \textbf{illustrative, not validated}. Whether systems built on this retained-state architecture
...
This is a computational architecture, not a therapeutic intervention.
...
In this framing, collapse and commitment are distinct: collapse is a point event at the output boundary, whereas \emph{commit inertia} is trajectory-level persistence of prior selections across turns. The $\phi$ mapping and non-destructive projection reduce this inertia by keeping alternatives available after each output step.
This frame-first design is intentionally structural: by preserving parallel candidate frames after each output step, systems reduce the need for unilateral early forcing in subsequent turns. Accordingly, structural readings are frame-conditional in operation: the same textual surface can map to different relation-structures under different active frame sets, without logical contradiction. We treat this as a condition-bounded architectural implication that downstream papers should test empirically, rather than as a universal interpersonal claim.
...
constructs an explicit state $S$ that can be preserved and transformed by retained-state
operators across dialogue turns. The distinction is between \textit{generating}
ambiguity (textual enumeration) and \textit{maintaining} ambiguity (state persistence). The point is not that no local judgment occurs inside the system; it is that retention itself need not be implemented as repeated full branchwise comparative evaluation during evidence accumulation.
...
We release deterministic code, fixed settings, and tracked artifacts for the
rule-based mapping and operator-validation reruns. For the main-text LLM table,
the package releases archived prompts, saved outputs, a sentence-level manifest,
and a local audit script that reconstruct the reported summaries rather than
replaying live model calls. The full environment specification and artifact map
are provided in \texttt{reproducibility.md}.

\subsection{Broader Impact and Risk Mitigation}

The illustrative psychological-support framing in Section~\ref{sec:application}
raises asymmetric-risk concerns: a system that preserves ambiguity can still be
misread as therapeutic guidance, can surface multiple harmful interpretations
without adequate guardrails, or can encourage over-reliance in emotionally
sensitive settings. The intended contribution of Phi is therefore architectural
and diagnostic, not clinical. Risk mitigation for this line includes keeping the
application framing explicitly non-therapeutic, avoiding claims of treatment
benefit, pairing any deployment-facing use with human oversight and domain
policies, and treating downstream evaluation in sensitive settings as a separate
ethics-reviewed task rather than as a consequence of the present results.
...
with a hybrid approach combining rule-based extraction and LLM-assisted ambiguity
enumeration.

The reported evaluation surface shows that the mapping can preserve interpretive
multiplicity under the tested extraction conditions: mean state entropy
$H = 1.087$ bits across all categories, compared to $H = 0$ for
collapse-based baselines.
This does not reject collapse itself; it targets premature collapse and preserves explicit commitment at the output boundary when conditions support reliable resolution.
At the explanatory level, the same explicit state structure supports later separation of retained state from output commitment rather than forcing that distinction to be reconstructed implicitly.
In this setup, improvements come from reducing unnecessary early commitment and preserving reversible update paths, not from adding architectural complexity. Read through the narrow non-evaluative organizing lens, Phi's role is to formalize how retained multiplicity can be updated as one structured state rather than by repeated full branchwise comparative evaluation at each step. That is a structural claim about the mapping and update design, not a universal runtime theorem.
Consistency Statement: Unsupported interpretations are never injected at a fixed evidence state. Hypothesis-set expansion is evidence-gated, and unobserved possibilities remain unresolved. Evidence in this paper is restricted to explicit input, provided context, and declared retrieval outputs.

The framework provides the missing algorithmic component of the prior retained-state formulation:
the transformation from text to the state space on which retained-state operators act. In the
current package, that contribution is supported by deterministic rule-based and
operator-validation reruns together with archived transcript-audit support for the
implicit-ambiguity cases. This is enough to make the bridge explicit and auditable
without claiming a fully rerunnable live hybrid implementation for every ambiguity
category in the current release.
...
\bibitem{anon2025framework}
Anonymous. (2025).
Prior retained-state framework paper omitted for blind review.
...
\section{Retained-State Operator Design Principles and Definitions}
\label{app:operators}

This appendix details design principles and formal definitions for the retained-state framework's
state-transition machinery. We distinguish (i) core transition operators
used for state evolution and (ii) metadata functions used for representation
management. While the $\phi$ mapping (main text) handles text-to-state
transformation, these components govern state-to-state processing.

\subsection{Design Principles for Non-Collapsing Operators}
\label{app:principles}

Transforming retained states risks destroying the multiplicity that this framework preserves.
We identify four principles that operators must satisfy to maintain non-collapse.
These principles derive from the structure of the retained state space itself.
...
principle from the retained-state foundations. Contradictions are structure, not
...
We define four core transition operators that have been empirically validated
to preserve non-collapse (§\ref{app:validation}), plus one state-level
calibration check and three metadata functions.

\subsubsection{Core Transition Operators (Empirically Validated)}

The following core transition operators have been validated on 180 single states,
...
\paragraph{State-level Calibration ($\sigma_{\mathrm{state}}$).}
Applies a uniform multiplicative scaling to all weights (state-level calibration check):

\begin{equation}
\sigma_{\mathrm{state}}(\mathcal{S}) = \left\{
  \left(v_i, c_i, w_i - b \cdot \frac{w_i}{\max_j w_j}\right)
\right\}
\end{equation}

This operation multiplies all weights by the same factor
$\left(1 - b/\max_j w_j\right)$.
Therefore, after normalization for entropy evaluation, the induced probability
distribution is unchanged and $\Delta H = 0$. We retain this as a
scale-equivariance calibration operator, not as an entropy-changing update rule.
This $\sigma_{\mathrm{state}}$ is separate from item-level strengthen operators
used in later implementation papers.
...
\subsubsection{Metadata Functions (Representation Utilities)}

The following three functions preserve weights by construction and primarily
augment representation metadata rather than perform transition dynamics.
They are included as extensible utilities.
...
Different components require different data structures for validation. 
Weight-transforming transitions ($\delta$, $\sigma_{\mathrm{state}}$, $\tau$) operate on 
...
exercise their core function. Metadata functions that neither modify weights 
...
information loss. For the transition components in this appendix, this bound 
is supported analytically: $\sigma_{\mathrm{state}}$ applies an identical multiplicative 
...
For $\epsilon = 0.1$ bits, a three-component 
chain $\delta \circ \tau \circ \rho$ is bounded by 
...
\begin{itemize}
  \item \textbf{$\sigma_{\mathrm{state}}$ (State-level calibration)}: 
    $\sigma_{\mathrm{state}}(c\mathcal{S})_i = c w_i - b \cdot c w_i / c\max w 
    = c(w_i - b \cdot w_i/\max w) = c \cdot \sigma_{\mathrm{state}}(\mathcal{S})_i$. 
...
All core transitions and metadata functions satisfy Principle 2 analytically.
...
\midrule
\multicolumn{5}{l}{\textit{Empirically validated core transitions:}} \\
$\delta$ (Dampening)\textsuperscript{\ddag}     & $\approx$\textsuperscript{*} & \checkmark & -- & -- \\
$\sigma_{\mathrm{state}}$ (Calibration)\textsuperscript{\ddag} & \checkmark & \checkmark & -- & -- \\
$\tau$ (Hold/Defer)\textsuperscript{\ddag}      & \checkmark & \checkmark & -- & -- \\
...
\midrule
\multicolumn{5}{l}{\textit{Metadata functions (representation utilities):}} \\
...
\textsuperscript{\S}$\rho$ only adds context metadata and does not modify weights, 
so entropy preservation holds by construction; empirically confirmed across all 
validation runs (§\ref{app:validation}).
...
For implementation, a compact transition chain
$\pi \circ \kappa \circ \delta \circ \tau$ is typically sufficient,
with $\phi$ (main text) providing the input transformation from text to
initial state. Optional metadata functions ($\rho, \alpha, \iota$) can be
inserted where context tracking or cross-context labeling is required.
Algorithm~\ref{alg:pipeline} shows this compact pattern with optional
positioning. Each validated transition preserves the interpretive multiplicity
established by $\phi$, enabling non-collapsing inference through repeated
updates.
...
\textbf{Data}: Three datasets were constructed using the Phi
...
\textbf{Operator--data matching}: Weight-transforming transitions 
($\delta$, $\sigma_{\mathrm{state}}$, $\tau$) are tested on single states. The integration 
...
\begin{itemize}
    ...
    \item \textbf{Principle-satisfying}: Proportional transformation 
          ...
          $\sigma_{\mathrm{state}}$: $w'_i = w_i - b \cdot w_i/\max w$)
\end{itemize}
...
and $\sigma_{\mathrm{state}}$ at four bias levels ($b \in \{0.05, 0.10, 0.15, 0.20\}$). 
...
operators ($\delta$ v2, $\sigma_{\mathrm{state}}$, $\tau$, $\kappa$, $\pi$, shown 
in green) achieve 0\% collapse. Operators $\kappa$ and $\pi$ 
substantially increase entropy through integration and persistence 
mechanisms. The near-zero bars for $\sigma_{\mathrm{state}}$ and $\tau$ are expected:
$\sigma_{\mathrm{state}}$ is a calibration check, and $\tau$ is an intentional hold step.}
\label{fig:operators}
\end{figure}
...
\midrule
$\sigma_{\mathrm{state}}$ ($b=0.10$) & single & 180 & 0 (0\%) & $0.000$\textsuperscript{\dag} \\
$\sigma_{\mathrm{state}}$ ($b=0.20$) & single & 180 & 0 (0\%) & $0.000$\textsuperscript{\dag} \\
\midrule
...
\textsuperscript{\dag}$\sigma_{\mathrm{state}}$ applies an identical multiplicative 
...
\begin{enumerate}
    ...
    \item \textbf{Dampening increases entropy}: $\delta$ v2 shifts 
          ...
          empirically. Here, entropy increase denotes redistribution
          across already extracted interpretations, not insertion of
          unsupported new interpretations.
    
    \item \textbf{State-level calibration is entropy-invariant}: 
          $\sigma_{\mathrm{state}}$ yields mean $\Delta H \approx 0$ bits by design,
          providing a scale-equivariance check without changing the normalized
