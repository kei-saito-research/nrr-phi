...
\textbf{Series hub}: A cross-paper index for the NRR series is available at
\url{https://github.com/kei-saito-research/nrr-series-hub}.
\end{abstract}
...
distinct from the item-level strengthen/dampen operators
($\sigma_{\text{item}}, \delta_{\text{item}}$) used in NRR-IME/Transfer/Coupled.
The symbol family is consistent; the operation level differs.
...
interpretation extraction using three models: ChatGPT, Gemini, and Claude.\footnote{Experiments used free-tier web interfaces (January 2026); exact model build IDs were not exposed.} 
Each model was prompted to enumerate all possible interpretations for 28 English 
sentences (8 epistemic + 10 lexical + 10 structural). The full prompts and 
raw model outputs are available in the project repository (see \texttt{prompts/}).
...
We release code, fixed settings, and primary logs for the reported mapping and operator-validation runs.
The executable commands fix the processing mode, input files, and output targets, with no temperature-based sampling in these offline scripts.
Model-related prompt artifacts are published as repository snapshots, and deterministic rule-based outputs are regenerated from the same scripts.
The full environment specification and artifact map are provided in \texttt{nrr-phi/reproducibility.md}.
...
At the explanatory level, the same explicit state structure also helps prevent unmarked collapse by preserving whether a resolved form remains a projection from retained state rather than a destructive replacement of it.
